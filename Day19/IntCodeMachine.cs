...
namespace AoC2019
{
    class IntCodeMachine
    {
        ...
        public event IcmInputReader InputReader;
        public event IcmOutputWriter OutputWriter;
        ...
        public void Reset(bool quick = false)
        {
            if (!quick)
                code.Reset();
            idx = 0;
        ...
        public bool Start (int[] input, out int output, bool quick = false)
        {
            Reset(quick);
        ...
        {
            // System.Console.WriteLine($"[{name}] Write: {index} {mode} {value}");
            switch (mode)
            {
                case MOD_POS:
